...
import TaskViewSwitcher, {
  type TaskViewMode,
} from "../../components/TaskViewSwitcher/TaskViewSwitcher";
...
export default function TasksPage() {
  ...
  const beginDrag = (event: ReactPointerEvent<HTMLElement>, task: WorkItem) => {
    ...
    if (
      ...
      event.target.closest("button, select, input, textarea, label, a")
    )
      return;
    ...
    const applyLanding = () => {
      ...
      const columnTasks = tasks.filter(
        (item) => item.status === landing.status,
      );
      ...
      const target =
        from >= 0
          ? reorder(
              columnTasks,
              from,
              landing.index > from ? landing.index + 1 : landing.index,
            )
          : [
              ...columnTasks.slice(0, landing.index),
              moved,
              ...columnTasks.slice(landing.index),
            ];

      setTasks([
        ...others,
        ...target.map((item) => (item.id === task.id ? moved : item)),
      ]);
    };
    ...
    const onUp = async () => {
      ...
      if (landing.status === task.status && landing.position === task.position)
        return;
    ...
    <Page className="page-stack">
      ...
      {loading ? (
        ...
        <TaskTable
          tasks={tasks}
          statuses={statuses}
          onEdit={(task: WorkItem) => navigate(`/tasks/${task.id}/edit`)}
